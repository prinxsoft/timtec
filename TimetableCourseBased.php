...
{	
	...
	if ($operation==0){
			...
			$mysql_0 = "SELECT distinct CONCAT(s.title,' ', s.initials,' ', s.staffName) AS fullname, d.deptCode, c.CourseCode,  c.CocoordinatorSP, ce.CoursePeriod, ce.ExamDayId, ce.Hall, ce.HallId, ce.ExamType	FROM coordinator c, courseexamperiod ce, staff s, department d where s.staffDeptId=d.deptId and ce.CourseCode =c.CourseCode  and c.CocoordinatorSP=s.staffSp and s.active=1 and  c.Semester=1 and c.Session='2020/2021' ;";
	...
	elseif ($operation==15){
			//echo("<ul>");
			$i=0;
			$mysql_0 = "SELECT c.CourseCode, c.coordinatorSp,  c.IsCoordAvailable, c.CocoordinatorSP, c.Semester, c.Session, CONCAT(s.title,' ', s.initials,' ', s.staffName) AS fullname, s.staffName, d.deptCode FROM coordinator c, staff s, department d where s.staffDeptId=d.deptId and s.active=1 and s.staffSp=c.coordinatorSp  and c.Semester=1 and c.Session='2020/2021' ;";
			$result_0 = $db->query($mysql_0)or trigger_error($mysqli->error."[$mysql_0]");
			if($result_0) {
				while($row_d = $result_0->fetch_assoc())
				{	
					...
					$mysql_1="SELECT  CoursePeriod, ExamDayId,  Semester, Session, Hall, HallId, ExamType FROM courseexamperiod where Semester=1 and Session='2020/2021' and CourseCode like '%$CourseCode%' limit 0,1;"; 
	...
	elseif ($operation==9){
			echo("<ul>");
			$i=0;
			$mysql_0 = "SELECT DISTINCT scheduleDate, examType,hall, period FROM `staffroaster`;";
			$result_0 = $db->query($mysql_0)or trigger_error($mysqli->error."[$mysql_0]");
			if($result_0) {
				while($row_d = $result_0->fetch_assoc())
				{	
 ...
 //unassigned invigilators do this first incrementally by 1 or looop over same way
	elseif($operation==2) 
	{
			...
			///if($checkCounter>0){
					...
					//do{
						if($num<>0){	
							...
							{	
									$bound=6; //the last max slot is the bound here
