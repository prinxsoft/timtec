...
<?php include("pagination.php"); ?>
    ...
    <?php
		...
		$timtecalloc = "SELECT  s.staffRoasterId, s.staffSp,s.staffName,  s.staffDeptCode,s.scheduleDate, s.ExamType, s.Hall,  s.hallCatTimtec,s.Period, s.Presence, s.hasObserver FROM staffroaster s, observerallocation o where s.day=o.day and s.hallCatTimtec=o.hallGroup and s.mainPeriod=o.observerPeriod and length(s.staffSP)>1 and s.scheduleDate<=CURRENT_DATE and s.Presence=-1 order by s.scheduleDate desc,s.staffRoasterId,o.observerPeriod;" ;
...
<?php include("pagination.php"); ?>
				...
				while($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){
					...
					<tr>
							...
							<td><div>
							<input type="radio" name="<?php  echo $NamePresence;?>" <?php  if (isset($Presence) && $Presence==2) echo "checked";?>
									value="2">Present </br>
									<input type="radio" name="<?php  echo $NamePresence;?>"
									<?php  if (isset($Presence) && $Presence==1) echo "checked";?>
									value="1">Is Late </br>
									<input type="radio" name="<?php  echo $NamePresence;?>"
									<?php if (isset($Presence) && $Presence==0) echo "checked";?>
									value="0">Absent</br>
									</div> 
									</td>
					</tr>
					...
					$mycount++;
						...
						?>
					<tr>  <td colspan="4"><input type="submit" name="submit" class="submit" value="Mark Attendance" /></td><td colspan="3"><a href="<?php  echo $link;?>"> <input type="submit"  class="submit" name="finalize" value="Finalize" /></a></td><td colspan="2"><a href="ObserverAttendance.php" class="submit"><font color="#FFFFFF"> Verify</font></a></td></tr></tr>
				<?php }
				else{
				?>
				<tr>
				<td  colspan="9"><p><?php echo "No Invigilation Schedule Alloted yet| Not yet Exam Date, contact TIMTEC! ";?></td></tr>
				<?php
				}
			...
			for ($i=0; $i<$count; $i++){
			 ...
			 if(isset($_POST["$NamePresence"]) ){
				...
				$sql_post = "UPDATE staffroaster SET presence =$Presence, markedBy='$staffNmb', dateMarked=now() WHERE staffRoasterId =$id ;";
